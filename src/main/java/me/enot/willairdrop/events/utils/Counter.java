...
public class Counter {
    ...
    public void stopWB() {
        ...
        if (!this.drop.getHologram().isDeleted()) {
            this.drop.getHologram().clearLines();
            ...
            if (Settings.debug()) {
                Bukkit.getConsoleSender().sendMessage("Hologram is deleted, lines doesn`t be setted.");
                Bukkit.getConsoleSender().sendMessage("DROP LOCATION: " + drop.getLocation().toString());
                Bukkit.getConsoleSender().sendMessage("DROP LOOT: " + drop.getLoot().getLootName());
            }
        }
    }
}
