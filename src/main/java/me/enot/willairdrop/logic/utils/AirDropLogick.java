...
public class AirDropLogick {
    ...
    public static boolean placeIsValid(Corner corner) {
        ...
        if (Settings.debug()) {
            Bukkit.getConsoleSender().sendMessage("Тип для спавна " + loc.getBlock().getType().toString());
            Bukkit.getConsoleSender().sendMessage("Координаты X " + loc.getBlockX() + " Y " + loc.getBlockY() + " Z " + loc.getBlockZ());
            Bukkit.getConsoleSender().sendMessage("Блоки по всей высоте ");
            for (int i = 0; i <= 256; i++) {
                Bukkit.getConsoleSender().sendMessage("" + i + " = " +
                        (new Location(Bukkit.getWorld(Settings.getSpawnWorld()), corner.getX(), i, corner.getZ()))
                                .getBlock().getType().toString());
            }
            Bukkit.getConsoleSender().sendMessage("" + !Settings.getBlockedMaterials().contains(loc.getBlock().getType()));
        }
    ...
    public static void doAnimation(AirDrop drop) {
        ...
        if (Settings.debug()) Bukkit.getConsoleSender().sendMessage("dropY: " + drop.getLocation().getY() + " hologramY: " + location.getY());
        ...
        if (Settings.debug()) Bukkit.getConsoleSender().sendMessage(ChatColor.RED + "Hologram animation до начала анимации " + hol.getY());
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (hol.getLocation().getY() > drop.getLocation().getY()) {
                    if (Settings.debug()) Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "while " + hol.getLocation().getY()+ " > " + drop.getLocation().getY());
                    hol.teleport(loc);
                } else {
                    if (Settings.debug()) Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "while " + hol.getLocation().getY()+ " < " + drop.getLocation().getY());
                    if (Settings.debug()) Bukkit.getConsoleSender().sendMessage("Удаление голограммы ");
                    spawnFireworks(hol.getLocation());
                    hol.delete();
                    cancel();
                    if (Settings.debug()) Bukkit.getConsoleSender().sendMessage("Установка сундука ");
